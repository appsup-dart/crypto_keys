...
void main() {
  group('KeyDeriver operator', () {
    test('both parties derive identical key material', () {
      ...
      final aZ = aDeriver.deriveKey(
        KeyDeriverParams.ecdh(peerPublicKey: b.publicKey!),
      );
      final bZ = bDeriver.deriveKey(
        KeyDeriverParams.ecdh(peerPublicKey: a.publicKey!),
      );

      final aKey = SecretBytes(aZ)
          .createKeyDeriver(kdfAlg)
          .deriveKey(
            KeyDeriverParams.concatKdf(keyBitLength: 256, otherInfo: otherInfo),
          );
      final bKey = SecretBytes(bZ)
          .createKeyDeriver(kdfAlg)
          .deriveKey(
            KeyDeriverParams.concatKdf(keyBitLength: 256, otherInfo: otherInfo),
          );
    ...
    test('derivation is deterministic and parameter-sensitive', () {
      ...
      final differentAlg = kdfDeriver.deriveKey(
        KeyDeriverParams.concatKdf(
          keyBitLength: 256,
      ...
      final differentU = kdfDeriver.deriveKey(
        KeyDeriverParams.concatKdf(
          keyBitLength: 256,
          otherInfo: _buildOtherInfo(
            algorithmId: Uint8List.fromList('kdf-a'.codeUnits),
      ...
      final differentV = kdfDeriver.deriveKey(
        KeyDeriverParams.concatKdf(
          keyBitLength: 256,
          otherInfo: _buildOtherInfo(
            algorithmId: Uint8List.fromList('kdf-a'.codeUnits),
  ...
  group('PasswordKeyDeriver operator', () {
    ...
    test('derivation is deterministic and parameter-sensitive', () {
      ...
      final differentIterations = deriver.deriveKey(
        KeyDeriverParams.pbkdf2(
          salt: salt,
          iterations: 2000,
          keyBitLength: 256,
  ...
  group('SecretBytes HKDF operator', () {
    test('matches RFC 5869 HKDF-SHA256 test vector case 1', () {
      ...
      final deriver = SecretBytes(
        ikm,
      ).createKeyDeriver(algorithms.derivation.hkdf.sha256);

      final okm = deriver.deriveKey(
        KeyDeriverParams.hkdf(
    ...
    test('derivation is deterministic and parameter-sensitive', () {
      final deriver = SecretBytes(
        _hexToBytes('00112233445566778899aabbccddeeff'),
      ...
      final differentInfo = deriver.deriveKey(
        KeyDeriverParams.hkdf(
          salt: Uint8List.fromList([1, 2, 3]),
    ...
    test('invalid parameters throw', () {
      ...
      final emptySecretDeriver = SecretBytes(
        [],
      ).createKeyDeriver(algorithms.derivation.hkdf.sha256);
      expect(
